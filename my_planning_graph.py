...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for a in self.all_actions:
             an=PgNode_a(a)
             if an.prenodes.issubset(slevels):
                 for s in list(slevels):
                     if s in list(an.prenodes):
                         an.parents.add(s)
                         s.children.add(a)
                 self.a_levels[level].add(an)
    ...
    def add_literal_level(self, level):
        ...
        alevels=list(self.a_levels[level-1])
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        IsCons3 = False
        IsCons4 = False
        for e in act1.effect_add:
            if e in act2.effect_rem:
                IsCons1 =True
        for e in act1.effect_rem:
            if e in act2.effect_add:
                IsCons2 =True        
        for e in act2.effect_add:
            if e in act1.effect_rem:
                IsCons3 =True
        for e in act2.effect_rem:
            if e in act1.effect_add:
                IsCons4 =True      
        return (IsCons1 and IsCons4) or (IsCons2 and IsCons3)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        IsCons4 = False
        for e in act1.effect_rem:
            if e in act2.precond_pos:
                IsCons1 =True
        for e in act1.effect_add:
            if e in act2.precond_neg:
                IsCons2 =True        
        for e in act2.effect_rem:
            if e in act1.precond_pos:
                IsCons3 =True
        for e in act2.effect_add:
            if e in act1.precond_neg:
                IsCons4 =True   
        return IsCons1 or IsCons3 or IsCons2 or IsCons4
    ...
    def h_levelsum(self) -> int:
        ...
        for g in self.problem.goal:
            isReach=False
            for l in list(self.s_levels):
                for s in list(l):
                    if s.symbol==g and s.is_pos:
                        isReach=True
                        break
                if isReach:
                    break
